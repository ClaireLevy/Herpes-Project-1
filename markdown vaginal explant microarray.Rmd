...
withEntrez<-interestingProbes [! (is.na(interestingProbes$ENTREZ_GENE_ID)), ]
  


#checking the symbols for out-of-dateness and other unusual behaviors
library(HGNChelper)

geneCheck<-checkGeneSymbols(withEntrez$SYMBOL)
names(geneCheck)[1]<-"SYMBOL"


CheckedwithEntrez<-merge(geneCheck,withEntrez,by="SYMBOL")\

#many of these have no suggested symbol, they are mostly all LOC genes
noSuggested<-CheckedwithEntrez[which(is.na(CheckedwithEntrez$Suggested.Symbol)),]
...
#limiting the vector of interestingProbes to only those with approved symbols

CheckedinterestingProbes <- CheckedwithEntrez[!(is.na(CheckedwithEntrez$Suggested.Symbol)),]


#just looking at the upreg probes for the first contrast

#THERES AN na ON THE END OF THIS VECTOR, DONT KNOW WHY
