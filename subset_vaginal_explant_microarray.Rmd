...
```{r plot logFC heatmap,echo=FALSE,message=FALSE, warning=FALSE}


#this DEFINITELY wont work if you have all the other data in memory at the same time!!. Clear the environment first then load mergeMock

load(mergeMock.Rdata)

ggplot(mergeMock[1:100,], aes(x = mockSubtracted, y = ProbeID))+
  geom_raster(aes(fill = mockSubtracted))+
