...
save(RAW.lumi, file="output/RAW.lumi.Rdata")
```
...
pData<- read.table("input/explantMicroarrayPhenoData.txt",
...
save(complete.RAW.lumi,file="output/complete.RAW.lumi.Rdata")
```
...
save(QNTB.complete.RAW.lumi, file = "output/QNTB.complete.RAW.lumi.Rdata")
```
...
load("output/expressedProbes.lumi.Rdata")
```
...
save(design, file="output/design.Rdata")
```
...
save(fit2, file= "output/fit2eBayes.Rdata")
```
...
save(results, file= "output/decideTestsResults.Rdata")
...
save(resultsDFmelt,file = "output/resultsDFmelt.Rdata")
...
write.table(forExcelShortGeneIDBdata, file = "INNATE DB RESULTS/forExcelShortGeneIDBdata.txt", sep = "\t", row.names=FALSE)
...
load("output/CELLStt3.Rdata")#top table for cells V186.24 DE probes
load("output/design.Rdata")#explant design matrix
...
load("output/CELLSV186UP.Rdata")#list of UP reg DE probes for V186 3hr 8 hr 24hr
load("output/CELLSV186DOWN.Rdata")#list of DOWN reg DE probes for V186 3hr 8 hr 24hr
