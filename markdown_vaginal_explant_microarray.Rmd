...
GOChangeNamesAndFilter<-function(df){
  names(df)<-c("GO Term","Corrected P Value","Source.Name")
  return(df)
  x<-df["Source.Name"=="biological process",]
  return(x)
}
# lapply to just the GO ORA dfs (the first 4 in the list)
shortIDBdata[1:4]<-lapply(shortIDBdata[1:4],FUN=GOChangeNamesAndFilter)
