...
makeShort <- function (df){
  ...
  select(Pathway.Name, Pathway.p.value,Source.Name)
}
#LMF says to use the non-adjusted p value for these results
...
GOChangeNames<-function(df){
  names(df)<-c("GO Term","P Value","Source.Name")
  return(df)
}
...
PWChangeNames<-function(df){
  names(df)<-c("Pathway Name","P Value", "Source.Name")
  return(df)
}
...
#I will just keep the entrez ID, symbol (called "name")and goTerms and cerebralLocalization
...
####Below is more filtering...for myself ###############
