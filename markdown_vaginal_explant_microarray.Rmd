...
#Downloaded the c2 gene set from MSigDB to use as my set of interest

# camera needs expression values (y)...

y<-exprs(expressedProbes.lumi)

#and an index vector, (use ids2indices function)

#provide a character vector of the entrez ids from y to ids2indices


identifiers = as.character(fData(expressedProbes.lumi)[,"ENTREZ_GENE_ID"])

index<-ids2indices(gene.sets = Hs.c2, identifiers = identifiers)
...
#get identical result if set inter.gene.cor to fixed value
cameraResult2<-camera(y=y, index=index, design = design, contrast = contr, inter.gene.cor = 0.01)

identical(cameraResult, cameraResult2)
####Now trying the same thing but using the DE probes from the epithelial cell MA as the set to compare against.
load("expressedProbes.lumi.Rdata")
load("CELLStt3.Rdata")
load("design.Rdata")
load("CELLSV186UP.Rdata")
load("CELLSV186DOWN.Rdata")
...
#Note that list makes a list with a single element and as.list makes each ID a different element (not what I want)
gene.sets<-list(as.character(CELLStt3$ENTREZ_GENE_ID))

index<-index<-ids2indices(gene.sets = gene.sets, identifiers = identifiers)
...
#put arguments into camera
V186.24cameraResult<-camera(y=y, index=index, design = design, contrast = contr )
...
```
Result from camera (competitive) gene set test with 
epithelial cell V186.24 DE probes as the gene set and the EXPLANT probes as the test genes. In the V186.24 treatment, the genes in this set are highly ranked relative to the genes not in the set.
