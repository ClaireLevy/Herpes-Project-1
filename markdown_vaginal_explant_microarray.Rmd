...
##### MERGE RESULTS WITH ENTREZ's FROM F DATA 

#add entrez ids to the melted p val and lfc filtered results df

ProbeIDandSymbol<-fData(RAW.lumi)[,c(1,4,7)]

save(ProbeIDandSymbol,file="ProbeIDandSymbol.Rdata")

#merge the melted results matrix with the entrez ids
interestingProbes<-merge(resultsDFmelt,ProbeIDandSymbol,
                         by="ProbeID")

#####  MAKE VECTOR OF DE PROBES WITH ENTREZ's ONLY
...
x<-fData (RAW.lumi) [ fData(RAW.lumi)$ProbeID == "1430239", ]
...
####### EXTRACTING INTERESTING PROBES FOR ANALYSIS

#function to extract the up and down regulated probes for each contrast with the entrez IDs

getProbes<-function(contrast,direction){
  interestingProbes[ interestingProbes$variable == contrast & interestingProbes$value ==  direction,
                     "ENTREZ_GENE_ID"]
}

#list of contrasts to pass to getProbes function
contrastList<-levels(interestingProbes$variable)

#down regulated probes
DownRegProbes<-lapply(contrastList, FUN=getProbes,"-1")

names(DownRegProbes)<-levels(interestingProbes$variable)

#upregulatedProbes
UpRegProbes<-lapply(contrastList, FUN=getProbes,"1")

names(UpRegProbes)<-levels(interestingProbes$variable)
...
#I will extract those for V186.24vsMock.24 (contrast3)from 
#the toptable that I made above (tt3) and exclude any probes
#witout entrez ids
...
##I put the above file into innateDB with the "subset of genes", "hypergeometric" and "no correction" options for the pathway ORA. I don't think it matters if pvals are included 
...
# lapply to just the GO ORA dfs (the first 2 in the list)
shortIDBdata[1:2]<-lapply(shortIDBdata[1:2],FUN=GOChangeNames)


#another function for the pathway ORA dfs
PWChangeNames<-function(df){
  names(df)<-c("Pathway Name","Corrected P Value")
  return(df)
}
#lapply to the PW ORA dfs
shortIDBdata[3:4]<-lapply(shortIDBdata[3:4],FUN=PWChangeNames)
