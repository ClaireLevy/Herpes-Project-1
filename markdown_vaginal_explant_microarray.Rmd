...
#merge in the fData with the results data 

V186.3ForIDB<-merge(ProbeIDandSymbol,V186.3ForIDB, by = "ProbeID")

#Get a df for just the upreg probes (lfc>0)
...
#Get a df for just the downreg probes (lfc<0)
...
### GETTING GENE SYMBOL LISTS FOR FH

#V186.24ForIDB has the symbol data so I can put that into genenames.org 

#top 100 UP DE probes
explant_top50_V186.24_UP<-V186.24ForIDB%>%
...
write.table(explant_top50_V186.24_UP,file="explant_top50_V186.24_UP.txt",row.names=FALSE,quote=FALSE)

#top 100 DOWN DE probes
explant_top50_V186.24_DOWN <- V186.24ForIDB%>%
  select(SYMBOL,logFC,adj.P.Val)%>%
  filter(logFC<0)%>%
  arrange(adj.P.Val)%>%
  slice(1:100)%>%
...
write.table(explant_top50_V186.24_DOWN,file="explant_top50_V186.24_DOWN.txt",row.names=FALSE,quote=FALSE)
...
geneFolder <- "J:/MacLabUsers/HLADIK SHARED/Projects/Herpes study/Herpes-Project-1/INNATE DB RESULTS/geneSpreadsheets"

geneFiles <- list.files(geneFolder, pattern=".txt")
...
#I will just keep the entrez ID, symbol (called "name")and cerebral localization
...
y<-melt(str_split(x$entrez, pattern=","))

colnames(y)<-c("singleEntrez","position")

z<-merge(y, x, by="position")

a<-z[,c("singleEntrez","name","fullname")]
...
#try with the UP data
ExtracellularGenesV186.24UP<-noCommas(genesV186.24UP)


#combine with the rest of the data (without commas in entrez)
#including a step to remove the rows with commas in the orig data

ExtracellularGenesV186.24UP<-genesV186.24UP %>%
  filter(str_detect(genesV186.24UP$entrez,",")==FALSE)%>%
  select(-goTerms)%>%
  rbind(ExtracellularGenesV186.24UP)

pander(ExtracellularGenesV186.24UP)

write.table(ExtracellularGenesV186.24UP, file = "ExtracellularGenesV186.24UP.txt", sep="\t", row.names=FALSE)
```
...
ExtracellularGenesV186.24DOWN<-genesV186.24DOWN %>%
  filter(str_detect(genesV186.24DOWN$entrez,",")==FALSE)%>%
  select(-goTerms)%>%
  rbind(ExtracellularGenesV186.24DOWN)


pander(ExtracellularGenesV186.24DOWN)

write.table(ExtracellularGenesV186.24DOWN, file = "ExtracellularGenesV186.24DOWN.txt", sep="\t", row.names=FALSE)
```

The one gene that they have in common:
